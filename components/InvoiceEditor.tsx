...
interface InvoiceEditorProps {
  ...
  folders?: Folder[]; // Receive available folders
}
...
const getStatusIcon = (status: InvoiceStatus) => {
  switch (status) {
    ...
    case InvoiceStatus.SENT: return <div className="w-2 h-2 rounded-full bg-blue-500" />;
...
const getFolderTextColorClass = (color?: string) => {
  ...
  return map[color || 'blue'] || 'text-blue-500';
};
...
export const InvoiceEditor: React.FC<InvoiceEditorProps> = ({ invoice, onChange, folders = [] }) => {
  ...
  const handleAiGeneration = async () => {
    ...
    try {
      ...
      onChange({
        ...
        client: {
          ...invoice.client,
          name: data.clientName || invoice.client.name,
          address: data.clientAddress || invoice.client.address
        },
        items: newItems,
      ...
      console.error("Erreur UI:", err);
      setAiError(err.message || "Impossible de générer la facture. Vérifiez votre clé API ou réessayez.");
    } finally {
  ...
  const toggleListening = () => {
    ...
    recognition.onerror = (event: any) => {
      ...
      setIsListening(false);
      setAiError("Erreur vocale: " + event.error);
    };
          ...
          < div className="bg-white md:bg-transparent rounded-xl p-1 md:p-0 relative z-20" >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={invoice.number} onChange={(e) => onChange({ ...invoice, number: e.target.value })} className="w-full border border-slate-300 rounded-lg text-base md:text-sm px-3 py-3 md:py-2.5 shadow-sm" />
              </div>
              <div>
